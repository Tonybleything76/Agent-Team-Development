...
contract = AgentContract(
    name="legal_agent",
    instruction="NDAs/MSAs/SOW redlines, clause library, negotiation notes."
)

async def main():
    app = MCPApp(name="legal_agent")
    configure_app(app)
    async with app.run():
        ...
        async with mcp_agent:
            ...
            output = (
              "Objective: Legal\n"
              "Body: draft output for legal_agent.\n"
            ...
            log_run({"agent":"legal_agent","artifact":path,"ok":ok,"issues":issues})
            print(path)
