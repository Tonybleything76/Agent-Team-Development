...
contract = AgentContract(
    name="oem_partner_agent",
    instruction="Partner selection, JMF/MDF, co-marketing; pipeline syncs."
)

async def main():
    app = MCPApp(name="oem_partner_agent")
    configure_app(app)
    async with app.run():
        ...
        async with mcp_agent:
            ...
            output = (
              "Objective: OEM_Partner_Leader\n"
              "Body: draft output for oem_partner_agent.\n"
            ...
            log_run({"agent":"oem_partner_agent","artifact":path,"ok":ok,"issues":issues})
            print(path)
