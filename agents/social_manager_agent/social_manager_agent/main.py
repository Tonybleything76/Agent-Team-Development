...
contract = AgentContract(
    name="social_manager_agent",
    instruction="Schedule posts, threads, UTM tagging, engagement reports."
)

async def main():
    app = MCPApp(name="social_manager_agent")
    configure_app(app)
    async with app.run():
        ...
        async with mcp_agent:
            ...
            output = (
              "Objective: Social_Media_Manager\n"
              "Body: draft output for social_manager_agent.\n"
            ...
            log_run({"agent":"social_manager_agent","artifact":path,"ok":ok,"issues":issues})
            print(path)
