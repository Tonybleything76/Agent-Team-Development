...
contract = AgentContract(
    name="it_agent",
    instruction="Provision accounts, backups, monitoring, incident response."
)

async def main():
    app = MCPApp(name="it_agent")
    configure_app(app)
    async with app.run():
        ...
        async with mcp_agent:
            ...
            output = (
              "Objective: IT\n"
              "Body: draft output for it_agent.\n"
            ...
            log_run({"agent":"it_agent","artifact":path,"ok":ok,"issues":issues})
            print(path)
