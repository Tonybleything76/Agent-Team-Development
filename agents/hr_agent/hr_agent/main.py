...
contract = AgentContract(
    name="hr_agent",
    instruction="Onboarding, SOPs, skills matrix, training assignments."
)

async def main():
    app = MCPApp(name="hr_agent")
    configure_app(app)
    async with app.run():
        ...
        async with mcp_agent:
            ...
            output = (
              "Objective: HR\n"
              "Body: draft output for hr_agent.\n"
            ...
            log_run({"agent":"hr_agent","artifact":path,"ok":ok,"issues":issues})
            print(path)
