...
contract = AgentContract(
    name="product_developer_agent",
    instruction="Build POCs/connectors/scripts, MCP tools, deployment utilities."
)

async def main():
    app = MCPApp(name="product_developer_agent")
    configure_app(app)
    async with app.run():
        ...
        async with mcp_agent:
            ...
            output = (
              "Objective: Product Developer\n"
              "Body: draft output for product_developer_agent.\n"
            ...
            log_run({"agent":"product_developer_agent","artifact":path,"ok":ok,"issues":issues})
            print(path)
