...
contract = AgentContract(
    name="ld_agent",
    instruction="Curricula, micro-videos, quick-reference guides, enablement packs."
)

async def main():
    app = MCPApp(name="ld_agent")
    configure_app(app)
    async with app.run():
        ...
        async with mcp_agent:
            ...
            output = (
              "Objective: Learning_Development\n"
              "Body: draft output for ld_agent.\n"
            ...
            log_run({"agent":"ld_agent","artifact":path,"ok":ok,"issues":issues})
            print(path)
