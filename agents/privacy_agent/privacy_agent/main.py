...
contract = AgentContract(
    name="privacy_agent",
    instruction="DPIAs, data maps, retention, consent logs; contract data clauses."
)

async def main():
    app = MCPApp(name="privacy_agent")
    configure_app(app)
    async with app.run():
        ...
        async with mcp_agent:
            ...
            output = (
              "Objective: Privacy\n"
              "Body: draft output for privacy_agent.\n"
            ...
            log_run({"agent":"privacy_agent","artifact":path,"ok":ok,"issues":issues})
            print(path)
